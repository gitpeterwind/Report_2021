...
\\
%\begin{verbatim}
F_{SO$_2$}=0.66*$O\_{DMS}$*Kw/3600*0.01*g*roa/p*Av  
%\end{verbatim}
\\
\\
where $O\_{DMS]$ is the surface water DMS concentration (in nanomol L$^{-1}$),
$g$ is standard gravity (9.807 m s$^{-2}$),
$roa$ is the ambient density of air (kg m$^{-3}$),
$p$ is ambient air pressure,
and $Av$ is Avogradro's number (6.023e+23 mol$^{-1}$).
