...
where the flux terms have units \ngN. The calculations of \Fb, \Ff, \Fd and \Fp are detailed in
 \citet{SimpsonDarras2021}.
...
\citet{SimpsonDarras2021} present the main sources for which
...
3Da3 in NFR, which together are roughly equivalent to
...
\citet{SimpsonDarras2021} recommended to retain the
...
\\
\\
The transfer velocity $K_{w}$ (given in cm hour$^{-1}$) is then calculated with the formula of \citet{Nightingale2000}, from the Schmidt number and the 10-meter wind speed $u_{10}$ (given in m~s$^{-1}$):
\\
\\
