...
accumulated ozone flux (\AFstY was used previously). The definitions of POD are 
discussed further in \citet{R2010:Fluxes}. See
...
and specific types of vegetation.  In the EU approaches \ce{O3} concentrations are taken directly from observations (at typically ca. 3~m height), or grid-average 3~m modelled values, for the AOT40 calculations. In the Mapping Manual
...
in accordance with the recommendations of \cite{MappingManual:Veg}, but in this report we will concentrate in this report on four definitions, derived from either `EU' approach or `UNECE' approaches:
...
In practice, it is very difficult to convert measured \ce{O3} from an EMEP observation site to the UNECE AOT40 values, since there are no data with which is to estimate the vertical gradient to get to upper-canopy \ce{O3}. Therefore, in the comparison of modelled and observed AOT40s in chapter \ref{ch:chapterStatus} and \ref{ch:Trends}, we have used the EU AOT definitions, since this approach is readily applicable to observed as well as modelled values. We do, however, present source-receptor calculations for the UNECE metrics \aotucf and \aotucc in Chapter~\ref{ch:appx_sr2019}.
