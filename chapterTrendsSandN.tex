...
\COMMENT{WAa: I have written something on aggregation of trends. Maybe more is needed.  We should probably explain the statistics done by NILU (thus not pyaerocom). Maybe we need to upload the statistics on the Github repository}
...
Thus, the selection of ozone metrics is decisive for the estimated trends (ref e.g. Lefohn - TOAR). The annual mean concentration used for evaluating other species is of little interest when studying ozone. A common procedure is to look at the trend in the probability distribution of ozone, e.g. by calculating trends for various percentiles of the distribution.  
...
\toprule
%\hline
               \multicolumn{4}{c}{Number of sites} & \multicolumn{4}{c}{Average Change (\ug $yr^{-1})$} & \multicolumn{4}{c}{Relative change (\% $yr^{-1}$)} \\
...
\COMMENT{ISSUES:\\ Explain diffs to Yttri et al 2021 and 2008-2018 trends better?\\
p-values of 1 look odd. Do we have any good words for this?}\\
}
\end{table}

%IN PROGRESS...
%Larger differences are seen in the absolute changes (which may reflect the %difficulties with the EC emissions mentioned in Sect.~\ref{ss:IssuesECOC}).
...
%caution should be made drawing firm conclusions from such short time series as those %presented here\COMMENT{ Yes!}, illustrated by the different reductions calculated for %levoglucosan for 2008--2018 and for 2010--2019.
...
\COMMENT{Dave got to here so far (12:40 Friday). Will continue later..}
...
\subsection{Trends in EC and OC, conclusion}
\label{ss:trendsECOCconc}

\COMMENT{DS to KE: I suggest simplifying the conclusions, making the main points but avoiding the detailed numbers (which have proved sensitive to time-windows). I'll write new text first, leaving the old below, so you can see my suggestions and we can agree...}


\COMMENT{NEW TEXT:\\}

For EC and OC we have calculated observed and modelled trends for 15 sites across Europe for the period 2010--2019. It should be noted that this 10-year period is rather short for the calculation of robust trends (and we show that slightly different time-windows produce different statistics), and typically only a limited number of sites showed trends that were statistically significant. However, some features of the trend analysis seem rather consistent, and we summarise the main points here. 
...
\COMMENT{\bigskip ORIG TEXT, DELETE if above is okay:\\}
...
\COMMENT{PUT TEXT FOR EXECUTIVE SUMMARY here and I can move it later:}\\

The observed trends in annual EC and wintertime OC (ca. 4~\%/yr) suggest that abatement measures
are having some success in reducing these components and PM in general. The reductions in levoglocosan also suggest a reduction in RWC emissions .....
Problems with condensables impact EC and especially OC ....
bla bla
