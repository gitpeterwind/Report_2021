...
Furthermore, Figure \ref{fig:pmExc_trends} presents for the period of 2001 to 2019 the changes in the number of sites at which the annual mean \PM[10]  and \PM[2.5] exceeded WHO AQG recommended levels (20 and 10 \ug respectively). Both observed and modelled with the EMEP MSC-W model results are shown. Only sites with observations for all the years from 2001 has been considered (the year 2000 has been excluded due to very few sites with observations available). Thus stricter criteria than for the trend assessment when 75\% data coverage over the time period was needed.

The number of sites with annual mean \PM[10]  and \PM[2.5] exceedances was decreasing from 9 to 5/6 sites between 2001 and 2007, then it remained unchanged at totally 3 exceedance sites from 2008 to 2018, going down to only 2 sites in 2019. The model agreement with observations is much better for \PM[2.5], while it mostly underestimates (or does not simulate) the number of exceedance days for \PM[10].
