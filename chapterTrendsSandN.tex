...
\begin{table}
\caption{\label{tab:pm25_stat} The estimated mean values of the absolute and relative trends in O$_3$ metrics based on all stations (observed and modelled). The corresponding 95\% confidence intervals of these mean values and the number of sites with significant trends are also provided.}
\begin{center}
\scalebox{0.65}{%
\begin{tabular}{ll|ccc|cccc|cccc}
\toprule
          &        & \multicolumn{3}{c}{Number of sites} & \multicolumn{4}{c}{Absolute change (ppb)} & \multicolumn{4}{c}{Relative change (\% $yr^{-1}$)} \\
Period & Parameter &           total & sign.(obs.) & sign.(mod.) &                    obs. &          conf.interval &  mod. &          conf.interval &                     obs. &         conf.interval &  mod. &        conf.interval \\
\midrule
2000-2019 & 75p &  93 &  35 &  68 & -0.17 & -0.20, -0.12 & -0.18 & -0.20, -0.16 & -0.32 & -0.39, -0.25 & -0.35 & -0.38, -0.31 \\
2000-2019 & 99p &  93 &  32 &  76 & -0.44 & -0.50, -0.37 & -0.41 & -0.44, -0.37 & -0.57 & -0.64, -0.48 & -0.61 & -0.64, -0.56 \\
\bottomrule
\end{tabular}}
\end{center}
\end{table}
...
%DAVE MOVED BIB STUFF TO END OF chapterTrendsECOC.tex
...
\hline
 & & & & \\
\multicolumn{5}{c}{Original using Karl Espen numbers, taken from text below} \\
& & & & \\
...
\end{tabular}\\
\parbox{12cm}{
\COMMENT{ISSUE:  p-values of 1 look odd.}\\
Notes:
...
%\subsection{Issues with inventories used in modelling}
\COMMENT{DS: MOVED section on issues due to CPOA into chapterTrendsSandN - also affects pmfine trends, and is anyway part of model setup.}
