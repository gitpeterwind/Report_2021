...
The observations used are all reported to EMEP and are openly available from the EBAS database (\url{http://ebas.nilu.no}. The time series have been selected based on statistical criteria as described in the next chapter  \ref{sec:Method} and based on the site location. Sites which are situated higher than around 2000 masl have been excluded (except DE0003 at 2007 masl is included)in addition to NO0042G since these are often above the boundarty layer and difficult for the model to visuel inspection of,
